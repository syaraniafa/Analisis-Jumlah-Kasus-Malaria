...
```{r}
# Prediksi PRR Unstandardize
yhat_prr2 <- exp(X_with_intercept %*% result$Estimate )

# Evaluasi PRR
mse_prr2 <- mean((y - yhat_prr2)^2)
rmse_prr2 <- sqrt(mse_prr2)
mae_pr2r <- mean(abs(y - yhat_prr2))
ss_res_prr2 <- sum((y - yhat_prr2)^2)
ss_tot_prr2 <- sum((y - mean(y))^2)
r2_prr2 <- 1 - (ss_res_prr2 / ss_tot_prr2)

cat("\nMSE PRR:", mse_prr2, "\n")
cat("RMSE PRR:", rmse_prr2, "\n")
cat("R² PRR:", r2_prr2, "\n")
```
...
data.frame(
  ...
  Rsquared = c("0.8178", pseudo_glm, r2_prr)
)
```
